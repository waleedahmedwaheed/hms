...
/*!
   ...
   */
f.fn.imagesLoaded=function(F){var D=this,B=D.find("img").add(D.filter("img")),y=B.length,E="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==",A=[];function C(){F.call(D,B)}function z(H){var G=H.target;if(G.src!==E&&f.inArray(G,A)===-1){A.push(G);if(--y<=0){setTimeout(C);B.unbind(".imagesLoaded",z)}}}if(!y){C()}B.bind("load.imagesLoaded error.imagesLoaded",z).each(function(){var G=this.src;this.src=E;this.src=G});return D};var w=function(y){if(n.console){n.console.error(y)}};f.fn.isotope=function(z,A){if(typeof z==="string"){var y=Array.prototype.slice.call(arguments,1);this.each(function(){var B=f.data(this,"isotope");if(!B){w("cannot call methods on isotope prior to initialization; attempted to call method '"+z+"'");return}if(!f.isFunction(B[z])||z.charAt(0)==="_"){w("no such method '"+z+"' for isotope instance");return}B[z].apply(B,y)})}else{this.each(function(){var B=f.data(this,"isotope");if(B){B.option(z);B._init(A)}else{f.data(this,"isotope",new f.Isotope(z,this,A))}})}return this}})(window,jQuery);(function(a){function b(){jQuery(".masonry-items").each(function(){var j=jQuery(this);var e=j.data("maxitemwidth");if(!e){e=370}var c=Math.ceil(((j.width()+(parseInt(j.css("marginLeft"))*2))/120)*100-(parseInt(j.css("marginLeft"))*2));var i=parseInt(j.children("div").css("marginRight"))+parseInt(j.children("div").css("marginLeft"));var f=Math.ceil(c/e);var g=(f-1)*i;var h=g/f;var d=Math.floor((c/f)-h+1);j.children("div").css({width:d+"px"});if(j.children("div").hasClass("isotope-item")){j.isotope("reLayout")}})}if(jQuery().isotope){jQuery(".masonry-items").each(function(){var c=jQuery(this);c.isotope({itemSelector:".masonry-item",transformsEnabled:true,layoutMode:"masonry"})});jQuery(".masonry-filters li a").click(function(){var d=jQuery(this).parents("ul.masonry-filters").data("related-grid");jQuery(this).parents("ul.masonry-filters").find("li a").removeClass("active");jQuery(this).addClass("active");var c=jQuery(this).attr("data-option-value");jQuery("#"+d).isotope({filter:c},function(){});return(false)});jQuery(".masonry-picker").on("change",function(f){var d=jQuery(this).data("related-grid");var g=a("option:selected",this);var c=this.value;jQuery("#"+d).isotope({filter:c},function(){})});b();jQuery(window).resize(function(){b()})}})(jQuery);