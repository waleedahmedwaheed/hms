                                ...
                                <div class="col-lg-12">
                                    ...
                                    <?php if(!isset($_GET["res_id"])){ ?>	
											<label>Reservation ID</label>
											 <select name="res_id" class="select-box" style="width:400px" required>
											<option value="">--Select--</option>
											<?php 
			$qry = "SELECT res_id,room_no,nic,mobile_no,guest_name FROM reserve where res_status=0 order by res_id";
		 mysql_select_db($database_dbconfig, $dbconfig);
		$results = mysql_query($qry);
		while($rowsu = mysql_fetch_assoc($results))
		{
		$res_ids 		= $rowsu["res_id"];
		$nic		 	= $rowsu["nic"];
		$mobile_no 		= $rowsu["mobile_no"];
		$guest_name		= $rowsu["guest_name"];
		?>
				<option value="<?php echo $res_ids; ?>" <?php if($res_id==$res_ids){ echo "selected";} ?>><?php echo $res_ids." - ".$guest_name." - ".$nic." - ".$mobile_no; ?></option>
...
if (window.XMLHttpRequest) {
        ...
        xmlhttp.open("GET","view_rorders.php",true);
        xmlhttp.send();
